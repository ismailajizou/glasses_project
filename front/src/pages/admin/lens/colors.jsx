...
const LensColorsPage = () => {
  ...
  const handleDelete = async (item) => {
    try {
      await csrf();
      if (item.deleted_at) await http.post(`/lens/colors/${item.id}/restore`);
      else await http.delete(`/lens/colors/${item.id}`);
  ...
  if (!lensColors) return <LoadingSpinner />;
  return (
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative my-2 overflow-x-auto shadow-md sm:rounded-lg">
          <Table>
            ...
            <TBody empty={!lensColors.data.length}>
              {lensColors.data.map(({ id, name, code, deleted_at }) => (
        ...
        <Pagination
          from={lensColors.from}
          to={lensColors.to}
          total={lensColors.total}
          links={lensColors.links}
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post("/lens/colors/add", values);
              mutate();
              setIsOpen(false);
            } catch (error) {
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post(`/lens/colors/${editModal.item.id}`, values);
              mutate();
