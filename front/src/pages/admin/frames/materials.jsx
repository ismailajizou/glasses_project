...
const MaterialsPage = () => {
  ...
  const {
    ...
    error,
    mutate,
  } = useFetch("/frames/materials" + l.search);
  ...
  const handleDelete = async (item) => {
    try {
      await csrf();
      if (item.deleted_at)
        await http.post(`/frames/materials/${item.id}/restore`);
      else await http.delete(`/frames/materials/${item.id}`);
      setDeleteModal({ item: null, isOpen: false });
      mutate();
      showNotification({
        title: "Success",
        message: `Material has been ${
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative my-2 overflow-x-auto shadow-md sm:rounded-lg">
          <Table>
            ...
            <TBody empty={!materials.data?.length}>
              {materials.data.map(({ id, name, deleted_at }) => (
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post("/frames/materials/add", values);
              mutate();
              setIsOpen(false);
              showNotification({
                title: "Success",
        ...
        >
          {({ isSubmitting, isValid }) => (
            <Form>
              <InputField label={"Material name"} name="name" id="brand_name" />
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post(`/frames/materials/${editModal.item.id}`, values);
              mutate();
