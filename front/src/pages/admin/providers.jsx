...
const ProvidersPage = () => {
  ...
  const handleDelete = async (item) => {
    try {
      await csrf();
      if (item.deleted_at) await http.post(`/providers/${item.id}/restore`);
      else await http.delete(`/providers/${item.id}`);
      setDeleteModal({ item: null, isOpen: false });
      mutate();
      showNotification({
        title: "Success",
        message: `Provider has been ${
  ...
  if (!providers) return <LoadingSpinner />;
  return (
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative my-2 overflow-x-auto shadow-md sm:rounded-lg">
          <Table>
            ...
            <TBody empty={!providers.data.length}>
              {providers.data.map(({ id, name, deleted_at }) => (
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post("/providers/add", values);
              mutate();
              setIsOpen(false);
              showNotification({
                title: "Success",
      ...
      >
        <Formik
          ...
          onSubmit={async (values, { setErrors }) => {
            ...
            try {
              await csrf();
              await http.post(`/providers/${editModal.item.id}`, values);
              mutate();
              setEditModal({ isOpen: false, item: null });
              showNotification({
                title: "Success",
                message: `Provider has been edited`,
                variant: "success",
              });
            } catch (error) {
              console.error(error);
