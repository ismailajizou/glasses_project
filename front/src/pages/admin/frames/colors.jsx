...
const FrameColorsPage = () => {
  ...
  const {
    ...
    error,
    mutate,
  } = useFetch("/frames/colors" + l.search);
  ...
  const handleDelete = async (item) => {
    try {
      await csrf();
      if (item.deleted_at) await http.post(`/frames/colors/${item.id}/restore`);
      else await http.delete(`/frames/colors/${item.id}`);
      setDeleteModal({ item: null, isOpen: false });
      mutate();
      showNotification({
        title: "Success",
        message: `Color has been ${item.deleted_at ? "restored" : "deleted"}`,
        variant: "success",
      });
    } catch (err) {
      console.error(err);
    ...
    <AdminLayout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative my-2 overflow-x-auto shadow-md sm:rounded-lg">
          <Table>
            ...
            <TBody empty={!frameColors.data.length}>
              {frameColors.data.map(({ id, name, code, deleted_at }) => (
                <tr key={id} className="border-b bg-gray-800 border-gray-700">
                  ...
                  <td className="px-6 py-4 flex justify-start">
                    <button
                      onClick={() =>
                        setEditModal({ item: { id, name, code }, isOpen: true })
                      }
        ...
        <Pagination
          from={frameColors.from}
          to={frameColors.to}
          total={frameColors.total}
          links={frameColors.links}
      ...
      >
        <Formik
          ...
          initialValues={{ name: "", code: "#000" }}
          onSubmit={async (values, { setErrors }) => {
            setErrors({});
            try {
              await csrf();
              await http.post("/frames/colors/add", values);
              mutate();
              setIsOpen(false);
            } catch (error) {
        ...
        >
          {({ isSubmitting, isValid }) => (
            <Form>
              <InputField label={"Color name"} name="name" id="color_name" />
              <InputField
                type="color"
                label={"Color code"}
                name="code"
      ...
      >
        <Formik
          ...
          initialValues={{
            ...
            code: editModal.item?.code ?? "#000",
          }}
          onSubmit={async (values, { setErrors }) => {
            setErrors({});
            try {
              await csrf();
              await http.post(`/frames/colors/${editModal.item.id}`, values);
              mutate();
      ...
      >
        <p>
          ...
          {deleteModal.item?.deleted_at ? "restore" : "delete"} this color?
        </p>
      </ConfirmationModal>
    </AdminLayout>
  );
...
export default FrameColorsPage;
